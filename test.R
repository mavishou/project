...
args <- c("result.filter.rehead.merge", "0.05")
...
# tlen <- tmp[, 1]
# tread <- tmp[, 2]
# 可是unique这一步数据大了之后会很慢，猜测作者是因为这个原因，才用了这么大一堆来得到tlen and tread
#--------------------
...
nb = vglm(tread_fit ~ 1, posnegbinomial(), weights = twts_fit, 
          maxit = 200, trace = FALSE, offset = logmu,
          epsilon=10^(-3), half.stepsizing = F)
# model parameters
(khat <- Coef(nb)["size"]);
(muhat <- Coef(nb)["munb"]);
nbsize = khat * mu;
